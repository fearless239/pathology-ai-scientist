...
"""
    ...
    def _create_agent_for_stage(self, stage: Stage) -> ParallelAgent:
        ...
        if main_stage == 2:
            ...
            if not stage2_substages and 2 not in self.cfg.agent.get('enabled_stages', [1, 2, 3, 4]):
                stage2_substages = [s for s in self.stages if s.name.startswith('1_')]
        ...
        while self.current_stage:  # Main stage loop
            ...
            while current_substage:  # Sub-stage loop
                ...
                with self._create_agent_for_stage(current_substage) as agent:
                    agent._on_seed_completed = (
                        lambda: step_callback(current_substage, self.journals[current_substage.name])
                    ) if step_callback else (lambda: None)
                    ...
                    if self.stage_history:
                        ...
                        if prev_best:
                            active_journal = self.journals[self.current_stage.name]
                            if not any(node.id == prev_best.id for node in active_journal.nodes):
                                active_journal.append(prev_best)
                        else:
                            print(
                    ...
                    while True:
                        # Resume completed work through the same transition path.
                        if not self._check_stage_completion(current_substage)[0]:
