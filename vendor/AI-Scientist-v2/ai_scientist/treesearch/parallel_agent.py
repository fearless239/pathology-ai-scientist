...
class ParallelAgent:
    ...
    def _process_node_wrapper(
        ...
        try:
            ...
            if hasattr(process_interpreter, "trusted_metrics_for_node"):
                # Domain adapter owns the metric boundary. Never execute an LLM
                # metric parser after a host-validated training execution.
                if not child_node.is_buggy:
                    try:
                        child_node.metric = MetricValue(value={"metric_names": process_interpreter.trusted_metrics_for_node(child_node.code)})
                    except Exception as error:
                        child_node.is_buggy = True
                        child_node.parse_exc_type = type(error).__name__
                        child_node.parse_term_out = str(error)
                data_files = []
            if not data_files:
                logger.warning(
            ...
            if not child_node.is_buggy:
                try:
                    ...
                    while True:
                        ...
                        if hasattr(process_interpreter, "run_for_purpose"):
                            plot_exec_result = process_interpreter.run_for_purpose(plotting_code, purpose="plotting")
                        else:
    ...
    def step(self, exec_callback: ExecCallbackType):
        ...
        for i, future in enumerate(futures):
            try:
                ...
                # Never continue scheduling after losing ownership of a worker.
                # The context manager must clean up before another run is allowed.
                raise RuntimeError("Worker deadline exceeded; stopping the stage for cleanup and recovery")
            except Exception as e:
                ...
                if future.done() and future.exception() is None and (
    ...
    def cleanup(self):
        ...
        if not self._is_shutdown:
            ...
            try:
                # Capture process handles before shutdown clears the mapping.
                ...
                if processes:
                    ...
                    for process in processes:
                        ...
                        if process.is_alive():
                            raise RuntimeError("Worker cleanup could not be confirmed")
