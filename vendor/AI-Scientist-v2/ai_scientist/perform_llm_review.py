...
def perform_review(
    ...
    get_response_from_llm = query or globals()["get_response_from_llm"]
    if query is not None and num_reviews_ensemble != 1:
        raise ValueError('Injected query requires one review; native batch calls would bypass it')
