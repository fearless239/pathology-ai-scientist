...
def writeup_step(*, idea_text, combined_summaries_str, aggregator_code, plot_names, writeup_text, plot_descriptions_str, big_client=None, big_client_model=None, page_limit=8, query=None):
    """Shared native drafting step; dependencies are scoped to this call."""
    query = query or get_response_from_llm
    ...
    return extract_writeup(response), msg_history


def extract_writeup(response):
    match = re.search(r"```latex(.*?)```", response, re.DOTALL)
    if not match:
        raise ValueError("Upstream writer returned no complete LaTeX block")
    return match.group(1).strip()

def reflection_step(*, unused_figs, invalid_figs, reflection_page_info, check_output, big_model_system_message, msg_history, big_client=None, big_client_model=None, query=None, review=""):
    """Shared native reflection prompt and call, with optional external review."""
    query = query or get_response_from_llm
    reflection_prompt = f"""
...
"""
    ...
    if review:
        reflection_prompt += "\nIndependent review and evidence checks:\n" + review
    return query(msg=reflection_prompt, client=big_client, model=big_client_model, system_message=big_model_system_message, msg_history=msg_history, print_debug=False)
...
def perform_writeup(
    ...
    try:
        ...
        updated_latex_code, msg_history = writeup_step(idea_text=idea_text, combined_summaries_str=combined_summaries_str, aggregator_code=aggregator_code, plot_names=plot_names, writeup_text=writeup_text, plot_descriptions_str=plot_descriptions_str, big_client=big_client, big_client_model=big_client_model, page_limit=page_limit)
        ...
        for i in range(n_writeup_reflections):
            ...
            reflection_response, msg_history = reflection_step(unused_figs=unused_figs, invalid_figs=invalid_figs, reflection_page_info=reflection_page_info, check_output=check_output, big_model_system_message=big_model_system_message, msg_history=msg_history, big_client=big_client, big_client_model=big_client_model)
