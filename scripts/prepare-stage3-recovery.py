"""Offline, explicit stage-3 recovery; never launches agents or training."""
import argparse
import copy
import hashlib
import json
...
with task_lock(root):
    ...
    assert stage.name.startswith('3_'), 'Only interrupted stage 3 is supported'
    assert all(s.stage_number <= 3 for s in state['stages']), 'Later-stage recovery needs separate review'
    journal = state['journals'][stage.name]
    assert len(journal.nodes) > 1, 'No rejected method attempts to replace'
    ...
    assert any(n.code == baseline.code and n.is_buggy is False
               for name,j in state['journals'].items() if name.startswith('2_') for n in j.nodes)
    stage1 = {name: pickle.dumps(j) for name,j in state['journals'].items() if name.startswith(('1_', '2_'))}
    assert all(n.is_buggy is True and str(n.term_out).startswith('Generated experiment rejected before execution:') for n in journal.nodes[1:]), 'Only pre-execution rejected attempts may be archived by this recovery'
    ...
    report = {'prepared_at': datetime.now(timezone.utc).isoformat(), 'task_id': task['task_id'],
              ...
              'baseline_code_sha256': code_hash, 'parent_validation_accuracy': metrics['accuracy'],
              'archived_attempt_ids': old_ids, 'remaining_active_nodes': 1,
              'max_generated_attempts': 3, 'training_started': False, 'paid_calls': 0,
              'checks': ['baseline artifact hashes and predictions', 'stages1_and2 unchanged',
                         'checkpoint roundtrip', 'stage3 pending without model calls'],
              'applied': args.apply}
    if args.apply:
        ...
        status['message'] = '恢复准备完成，仍未运行；旧记录已备份，下一次从已验证调参结果继续方法实验。'
        ...
        (root/'stage3_recovery.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
